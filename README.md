...
In full mode, taxonomy resolution is a bounded ReAct loop rather than a pre-ranked candidate classifier. The chat model receives native read-only function tools for category-root/child navigation, model-chosen literal searches, exact node inspection, schema discovery, attribute inspection and enum-value lookup. Each tool observation is returned as a `role=tool` message and the model controls the next query, can paginate or backtrack, and finishes only after the selected leaf and every submitted schema/attribute/value ID have been observed. Code validates those IDs and exact source attribute pairs but does not choose the semantic winner. The older lexical scorer remains only as an offline or API-failure fallback and is not included as an online model hint.

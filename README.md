...
In full mode, taxonomy resolution uses two bounded ReAct transactions rather than a pre-ranked candidate classifier: the first commits a leaf category and the second receives that accepted result in a fresh context to resolve its attribute schema and mappings. If the first transaction cannot finish, a fresh model context receives the complete bounded taxonomy evidence and attempts recovery. A successful grounded category transaction is not challenged merely because it differs from the weak lexical availability candidate. An attribute failure gets a fresh model recovery attempt and can then fall back only the schema/mapping projection while preserving the accepted category. Schema discovery may select the best available attribute definition but never rewrites the leaf category decision. The availability path never uses sample IDs, product filenames or keyword-to-answer tables. Each ReAct task has its own independent 50-turn budget; neither task consumes or inherits the other's turns. Every turn exposes that task's remaining budget, and the last two turns of each task expose only its validated finish tool so one rejected finish can still be corrected. An identical deterministically rejected finish stops the phase on its second submission instead of consuming the rest of the budget.
...
Localized copy uses a writer pass followed by a factual-auditor pass and locale guards for en-US, ko-KR and pt-BR. The models author only the buyer-facing title, overview, highlights and fit note; code independently builds the machine appendix from verified taxonomy, attribute, SKU and media contracts, so prose revisions cannot mutate parseable identifiers. Runtime terminology is merged field by field: audited translations take priority, stable deterministic translations remain intact, and a valid draft can recover only otherwise unknown entries when the audit is partial or unavailable. If no trustworthy translation exists, the exact source value remains observable in the machine appendix instead of becoming an empty placeholder. Every description leads with two substantive shopper-facing paragraphs and natural feature highlights, followed by compact localized listing tables. Category IDs, platform attribute/value IDs, SKU IDs and Spec IDs remain parseable; localized source labels distinguish seller facts from platform mappings, while source-script residue is isolated to the machine appendix and reported by QA. Legible source-image tables use a domain-neutral cell protocol: the model decides their meaning, selected rows and columns, localized labels, notes and detail-image placement, while code checks exact source-cell references and renders only a complete grounded presentation. Unsupported numeric fields are repaired independently so one bad number no longer discards an otherwise valid localized draft.
...
A normal platform run uses all configured model endpoints. Ordinary transient throttles, including generic allocation-capacity throttles, retain bounded retries. Only a provider response that explicitly reports exhausted account allocation opens a run-local service circuit across capabilities; endpoint authorization and model-configuration failures remain scoped so another configured chat, image or video capability can still run. If one or more model endpoint variables are absent, the runtime records the configuration problem and continues through the deterministic availability fallback instead of returning an empty failed submission. `--offline` remains the explicit development mode.
...
The automated suite focuses on complete protocol smoke runs, deterministic tool boundaries, safety/data-integrity contracts and structurally valid delivery output. Semantic quality and generalization belong in holdout evals rather than unit tests that freeze one model trajectory, prompt wording, call count or sample answer. Resilience tests cover temporary allocation throttles versus permanent quota exhaustion, capability-scoped authorization failures, run-local service circuit breaking, malformed JSON, corrupt media, failed asynchronous tasks, unavailable video capabilities, factual CLI-boundary recovery and deadline-aware fallback.

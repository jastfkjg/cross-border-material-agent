...
## Agent autonomy and semantic decisions

- Do not grow the system by accumulating rules of the form “when keyword or phrase X appears, choose fix Y.” Such routing and repair tables make multi-round optimization brittle, obscure the agent's actual decision process, and overfit observed failures instead of solving the underlying class of problems.
- Let the model own open-ended semantic work whenever practical: interpreting evidence, diagnosing defects, selecting among valid alternatives, proposing implementation details, and choosing or revising a repair strategy. Give it grounded context, bounded tools, explicit outcome contracts, and actionable feedback rather than encoding the desired semantic conclusion in host logic.
- Keep deterministic host code focused on domain-wide invariants and execution boundaries: protocol and schema validity, authorization, safety, provenance and grounding, data integrity, resource limits, transactional behavior, and objective artifact checks. Validate what the agent produces without prescribing one semantic route to the result.
- When a new failure is observed, first improve the evidence available to the model, tool affordances, validation feedback, or the general decision loop. Add a deterministic special case only when the behavior is strategy-independent, remains correct for unseen inputs, and can be documented as a stable system contract.

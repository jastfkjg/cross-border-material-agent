...
- [taxonomy][hard] Control retrieval iteratively through the generic normalized collections: batch model-chosen filters when useful, combine or separate terms deliberately, follow category/schema/attribute/value refs, paginate when necessary, and revise the query after an empty, broad or weak observation. Do not require a pre-ranked lexical Top-N and do not assume the first page is complete.
